			...
			afterSync:function(){
				return  sequelize.transaction(function(t){
					let query = 'CREATE OR REPLACE FUNCTION "derivedInvoiceNumber"("Invoices")'+
								...
								'SELECT'+
								'CASE'+
									'WHEN $1."type"="Invoice" THEN $1."invoiceNumber"'+
									'WHEN $1."type"="CreditNote" THEN $1."invoiceNumber"'+
									'WHEN $1."type"="Proforma" THEN "P"||$1."id"'+
								'END'+
								'$$ STABLE LANGUAGE SQL';
					return sequelize.query(query, {transaction: t})
						.then(function(){
							let query = 'CREATE OR REPLACE FUNCTION "subTotal"("Invoices")'+
										...
										'SELECT $1."totalCharges" + $1."prepayment"'+
										'$$ STABLE LANGUAGE SQL';
							return sequelize.query(query, {transaction: t});
						})
						.then(function(){
							let query = 'CREATE OR REPLACE FUNCTION total("Invoices")'+
										'RETURNS double precision AS $$'+
										'SELECT $1."totalCharges" + $1."prepayment" + $1."tax"'+
										'$$ STABLE LANGUAGE SQL';
							return sequelize.query(query, {transaction: t});
						})
						.then(function(){
							let query = 'CREATE OR REPLACE FUNCTION due("Invoices")'+
										'RETURNS double precision AS $$'+
										'SELECT $1."totalCharges" + $1."prepayment" + $1."tax" - $1."payment"'+
										'$$ STABLE LANGUAGE SQL';
							return sequelize.query(query, {transaction: t});
						});
				});
