...
module.exports = function (sequelize, enums, config) {
	const DataTypes = sequelize.Sequelize;

	return sequelize.define('Invoice', {
		status: {
			type: DataTypes.ENUM({values: Object.keys(enums.InvoiceStatus)}),
			allowNull: false,
			defaultValue: enums.InvoiceStatus.Active.key
		},
		...
		// --- proforma fields --
		type: {
			type: DataTypes.ENUM({values: Object.keys(enums.InvoiceType)}),
			allowNull: false
		},
		...
		paymentType: {
			type: DataTypes.ENUM({values: Object.keys(enums.PaymentType)}),
			allowNull: true
		}
		...
		classMethods : {
			...
			generateCreditNote: function*(id, note, totalCharges) {
				...
				let creditMemoData = {
					...
					invoiceNumber,
					note,
					type: enums.InvoiceType.CreditNote.key,
					activationType: null,
					activationId: null,
			...
			generateProforma: function*(CompanyId, transactions, activationType, activationId, note) {
				...
				let isBGNomenclature = bgNomenclature.indexOf(company.countryOfReg) > -1;
				let hasVatNumber = company.vatNumber !== null && company.vatNumber !== '';
				let totalCharges = Transaction.calculateTotal(transactions);
				let isReverseCharged = (!isEUCountry || (isEUCountry && hasVatNumber)) && !isBGNomenclature;
				...
				let cacheInvoice = yield this.createWithCache({
					...
					tax: computedTax,
					date:  new Date().setHours(0, 0, 0, 0),
					type: enums.InvoiceType.Proforma.key,
					note,
					activationType,
					activationId,
					isReverseCharged
			...
			generateInvoice: function*(CompanyId, transactions, note, asOfDate){
				...
				let isReverseCharged = (!isEUCountry || (isEUCountry && hasVatNumber)) && !isBGNomenclature;
				let computedTax = isReverseCharged ? 0 : totalCharges * 0.2;
				let today = new Date(asOfDate + config.invoiceGraceDays*24*60*60*1000);
				...
				let invoice = yield this.createWithCache({
					...
					type: enums.InvoiceType.Invoice.key,
					total: totalCharges + computedTax,
					isReverseCharged
				});
