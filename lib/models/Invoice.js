	...
	var Invoice = sequelize.define('Invoice', {
		...
		instanceMethods : {
			...
			recalcSellerAddress: function*(){
				...
				let aux     = yield CompanyModel.findById(1,         {include: [{association: CompanyModel.associations.Wallet}]});
		...
		classMethods : {
			...
			createWithCache: function*(data){
				...
				return yield Invoice.create(_.extend(data, cacheData));
			},
			...
			generateCache: function*(companyId){
				...
				let company = yield CompanyModel.findById(companyId, {include: [{association: CompanyModel.associations.Wallet}]});
			...
			generateProforma: function*(CompanyId, transactions, activationType, activationId, note){
				...
				let invoice = yield Invoice.createWithCache({
					CompanyId,
					invoiceNumber: '',
					totalCharges,
				...
				invoice = yield Invoice.findById(invoice.id);
			...
			invoicePendingTransactions: function*(companyId, note, asOfDate) {
				...
				return yield db.Invoice.generateInvoice(companyId, pending, note, asOfDate);
			},
