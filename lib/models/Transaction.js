...
module.exports = function (sequelize, enums) {
	...
	const TransactionSide = enums.TransactionSide;
	const TransactionLinkType = enums.TransactionLinkType;
		...
		classMethods : {
			...
			get: function*(companyId, criteria) {
				...
				query.where = sequelize.and(
					...
					{
						CompanyId: companyId,
						type: TransactionType.Invoice.key
					}
				);
				query.include = [
					{
			...
			duplicateFromProforma: function*(transactions) {
				return yield transactions.map(t => {
					...
					newTransaction.type = TransactionType.Invoice.key;
