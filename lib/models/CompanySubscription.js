...
module.exports = function (sequelize) {
	...
	return sequelize.define('CompanySubscription', {
		status: {
			type: DataTypes.ENUM({values: Object.keys(CompanySubscriptionStatus)}),
			allowNull: false
		},
		typeName: {
		...
		classMethods: {
			*activate(id){
				...
				let totalNegative = 0;
				for(s of negativeSubscr){
					totalNegative = totalNegative+s.remaining;
					s.remaining = 0;
					yield s.save();
				}
			...
			*consume(CompanyId, typeIdent, amount, description, privateDescr){
				...
				for(let n=0; n< subscriptions.length; n++){
					...
					if(n==subscriptions.length-1){
						// last available subscription
						deductAmnt = totalRemaining;
					} else {
						deductAmnt = Math.min(s.remaining, totalRemaining);
					}

					s.remaining = s.remaining-deductAmnt;
					totalRemaining = totalRemaining-deductAmnt;
					if(s.remaining<0.0001){
						s.status = CompanySubscriptionStatus.Expired;
				...
				if(totalRemaining > 0.0001) {
					yield this.create({
						status: CompanySubscriptionStatus.Expired,
						CompanyId: CompanyId,
						typeIdent: typeIdent,
			...
			*getRemaining(CompanyId, typeIdent, includePending){
				let statuses = [CompanySubscriptionStatus.Active];
				if(includePending) statuses.push(CompanySubscriptionStatus.Pending);
				let subscriptions = yield this.findAll({
					where: {
						CompanyId,
						typeIdent,
			...
			/**
			 * Called periodically (@see scheduler.finishExpiredSubscriptions configuration) to mark expired auctions
			 ...
			 */
				*finishExpired(now) {
				...
				for(let sub of subscriptions){
					sub.status = CompanySubscriptionStatus.Expired;
					sub.save();
				}
			},
...
module.exports.CompanySubscriptionStatus = CompanySubscriptionStatus;
